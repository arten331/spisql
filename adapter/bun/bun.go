...
func Apply(sq *bun.SelectQuery, q *spisql.Query, m Mapping) *bun.SelectQuery {
	if q == nil {
		return sq
	}
	for _, f := range q.Filters {
		sq = applyFilter(sq, f, m)
	}
	for _, gr := range q.Groups {
		if frag, args, ok := Group(gr, m); ok {
			sq = sq.Where(frag, args...)
		}
	}
	...
	if q.Pagination.Limit > 0 {
		sq = sq.Limit(int(q.Pagination.Limit))
	}
	if q.Pagination.Offset > 0 {
		sq = sq.Offset(int(q.Pagination.Offset))
	}
	return sq
}
...
func applyFilter(sq *bun.SelectQuery, f *spisql.Filter, m Mapping) *bun.SelectQuery {
	frag, args, ok := Filter(f, m)
	if !ok {
		return sq
	}
	return sq.Where(frag, args...)
}
...
func inExpr(col string, values []string, negate bool) (string, []any, bool) {
	...
	return col + " " + op + " (?)", []any{bun.In(toAny(values))}, true
}
